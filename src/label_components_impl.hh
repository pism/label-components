...
//! Adds "foreground" and "attached" concepts to an "array".
template <typename ARRAY>
class Mask {
public:
  typedef typename ARRAY::value_type value_type;

  Mask(ARRAY &data, value_type foreground_threshold,
       value_type attached_threshold)
      : m_data(data), m_foreground_threshold(foreground_threshold),
        m_attached_threshold(attached_threshold) {
    // empty
  }
  ...
  inline bool is_foreground(int i, int j) const {
    return m_data(i, j) > m_foreground_threshold;
  }

  inline bool is_attached(int i, int j) const {
    return m_data(i, j) > m_attached_threshold;
  }

private:
  ARRAY &m_data;
  value_type m_foreground_threshold;
  value_type m_attached_threshold;
};
...
bool label(const mask &input, bool identify_isolated_patches, int min_label, array &output) {
    ...
    while (c < ncols) {
      if (input.is_foreground(r, c)) {
        ...
        while (c < ncols and input.is_foreground(r, c)) {
          ...
          if (identify_isolated_patches and input.is_attached(r, c)) {
